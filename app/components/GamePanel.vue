<template>
  <div class="absolute bottom-0 left-0 right-0 z-10 flex justify-center pb-4 px-4 pointer-events-none">
    <div class="card max-w-lg w-full pointer-events-auto shadow-lg">
      ...
      <!-- Session Complete -->
      <div v-else-if="phase === 'sessionComplete'" class="text-center space-y-3">
        ...
        <div class="flex gap-2">
          <button
            ...
            @click="copyScore"
          >
            <svg v-if="!copied" class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 12v8a2 2 0 002 2h12a2 2 0 002-2v-8"/><polyline points="16 6 12 2 8 6"/><line x1="12" y1="2" x2="12" y2="15"/></svg>
...
function buildShareText(): string {
  return [
    `\u{1F5FA}\u{FE0F} Di Mana? \u2014 ${formatDate()}`,
    `${props.sessionCorrect}/${props.sessionTotal} ${collectionLabel.value}`,
    `${scoreEmoji.value}`,
    `#DiMana`,
    `https://dimana.co.bn`,
  ].join('\n')
